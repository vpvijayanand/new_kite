...
def test_signal_service():
    ...
    try:
        ...
        with app.app_context():
            ...
            # Test signal generation for latest data
            signals = generator.generate_signals_for_latest_data()
            
            if signals:
                print(f"✅ Signal generation service: OK - Generated {len(signals)} signals")
                if signals:
                    latest_signal = signals[-1]
                    print(f"📊 Latest signal: {latest_signal.signal_type} at ₹{latest_signal.price}")
            else:
                print("✅ Signal generation service: OK - No recent signals (normal)")
